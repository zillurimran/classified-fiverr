...
@section('main')
            ...
            <div class="header-text mb-0">
                <div class="container">
                    <div class="text-center text-white">
                        <h1 class="">{{ucfirst($category->name)}}</h1>
                        <ol class="breadcrumb text-center">
                            <li class="breadcrumb-item"><a href="{{ route('home.page') }}">Home</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('blog') }}">Blogs</a></li>
                            <li class="breadcrumb-item active text-white" aria-current="page">{{ucfirst($category->name)}}</li>
                        </ol>
                    </div>
                </div>
            </div>
                            ...
                            <div class="product-tags clearfix">
                                <ul class="list-unstyled mb-0">
                                    @foreach ($tags as $tag)
                                        ...
                                        <li><a href="{{route('tag.blogs', $tag->id)}}" class="tagWiseSearch" data-id="{{ $tag->id }}">{{ $tag->name }}</a></li>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.authorWiseBlog').on('click', function(){
                let id = $(this).attr('data-id');
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
